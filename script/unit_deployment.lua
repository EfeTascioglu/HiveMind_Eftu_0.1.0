...
local get_max_pop_count = function(force_index)
  --if not data.max_pop_count[force_index] then -- Use this to prevent updating popcap every unit deployment step. I have disabled this so that players can dynamically change popcap.
  data.max_pop_count[force_index] = (game.forces[force_index].technologies["popcap"].level - 1) * settings.global["hivemind-increase-per-level"].value + settings.global["hivemind-starting-popcap"].value
  --end
